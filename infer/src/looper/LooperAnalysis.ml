  ...
  try
          ...
          (fun (dcp_src, edge_data, dcp_dst) ->
            ...
            let condition_norms =
              List.map edge_data.condition_norms ~f:(fun norm_set ->
                  EdgeExp.Set.map
                    (fun norm ->
                      let wrapped_norm = EdgeExp.T.Max (EdgeExp.Set.singleton norm) in
            ...
            let constraints =
              List.fold edge_data.constraints ~init:[] ~f:(fun acc (lhs_norm, dc_rhs) ->
                  ...
                  let map_value_rhs (rhs_norm, op, rhs_const) =
                    ...
                    match
                      (VFG.Map.find_opt lhs_node vfg_map, VFG.Map.find_opt rhs_node vfg_map)
                    with
              ...
              if EdgeExp.is_variable norm formals tenv then
                ...
                if LooperSummary.Resets.is_empty norm_updates.resets then (
                  ...
                  | _ ->
                      let msg =
                        F.asprintf
                          "Variable norm '%a' has no existing reset. Assuming Infinite variable \
                           bound"
                          EdgeExp.pp norm
                      in
    ...
    and transition_bound (src, (edge_data : DCP.EdgeData.t), dst) (cache : LooperSummary.cache) =
      ...
      | None -> (
          ...
          | [] ->
              ...
              (bound, cache)
          | bound_norms ->
              (* TODO: Adapt for sets of LBs *)
              let norm = EdgeExp.Set.min_elt (List.hd_exn edge_data.bound_norms) in
    ...
    let return_bound =
      ...
      | Tint _ ->
          ...
          in
          let return_norm =
            EdgeExp.T.Max (EdgeExp.Set.singleton (EdgeExp.T.Access return_access))
          in
          let ub_return_bound, cache =
            variable_bound ~bound_type:BoundType.Upper return_norm cache
          in
          let lb_return_bound, _ = variable_bound ~bound_type:BoundType.Lower return_norm cache in
          debug_log "[Return bound] [%a, %a]@," EdgeExp.pp lb_return_bound EdgeExp.pp
            ub_return_bound ;
