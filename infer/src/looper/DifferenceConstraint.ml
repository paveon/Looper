...
let make ?(const_part = Binop.PlusA None, IntLit.zero) lhs_norm rhs_norm = 
  ...
  match op with
  | Binop.PlusA _ | Binop.Shiftrt -> lhs_norm, Value (rhs_norm, op, rhs_const)
  | _ -> assert(false)
...
let rec is_constant (lhs_norm, rhs) = match rhs with
...
let rec same_norms (lhs_norm, rhs) = match rhs with
  | Value (rhs_norm, _, _) -> EdgeExp.equal lhs_norm rhs_norm
  | Pair (lower_rhs, upper_rhs) ->
    same_norms (lhs_norm, (Value lower_rhs)) &&
    same_norms (lhs_norm, (Value upper_rhs)) 


let is_reset dc = not (same_norms dc)


let rec is_decreasing (lhs_norm, rhs) = match rhs with
  | Value (_, op, const) -> (
    match op with
  ...
  | Pair (lower_rhs, upper_rhs) ->
    is_decreasing (lhs_norm, Value lower_rhs) &&
    is_decreasing (lhs_norm, Value upper_rhs)


let rec is_increasing (lhs_norm, rhs) = match rhs with
  | Value (_, op, const) -> (
    match op with
  ...
  | Pair (lower_rhs, upper_rhs) ->
    is_increasing (lhs_norm, Value lower_rhs) &&
    is_increasing (lhs_norm, Value upper_rhs)
...
let rec to_string (lhs, rhs) = 
  ...
  | Pair (lower_rhs, upper_rhs) ->
    F.asprintf "%a' <= [%s; %s]" EdgeExp.pp lhs 
      (value_rhs_to_str lower_rhs)
