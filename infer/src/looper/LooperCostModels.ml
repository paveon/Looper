...
let debug_log : ('a, Format.formatter, unit) format -> 'a = fun fmt -> 
  F.fprintf (List.hd_exn !LooperUtils.debug_fmt) fmt
...
module BoundsOf (Container : ContainerSignature) = struct
  let of_length: 
    ...
    fun args ~of_function ~complexity -> (
      ...
      let complexity = match List.hd_exn args with
      ...
      | EdgeExp.ValuePair.P (lb, ub) -> (
        if EdgeExp.equal lb ub then (
          EdgeExp.ValuePair.V (EdgeExp.T.Symbolic (complexity, lb))
        ) else (
          EdgeExp.ValuePair.P (
            EdgeExp.T.Symbolic (complexity, lb),
            EdgeExp.T.Symbolic (complexity, ub))
        )
      )
      in
...
type get_model_summary =
  LooperModelEnv.t -> ret:Ident.t * Typ.t -> LooperSummary.Model.t

module Call = struct
  let models = [
    ("strlen", BoundsOfCString.linear_length ~of_function:"strlen");
  ]
