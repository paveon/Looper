...
module JsonLooperPrinter = MakeJsonListPrinter (struct
  ...
  let to_string {loc; proc_name; looper_opt} =
    match looper_opt with
    | Some looper_summary ->
        let return_bound : string list option =
          match looper_summary.return_bound with
          | Some (EdgeExp.ValuePair.P (lb, ub)) ->
              Some [EdgeExp.to_string lb; EdgeExp.to_string ub]
          | Some (EdgeExp.ValuePair.V ret_bound) ->
              Some [EdgeExp.to_string ret_bound]
          | None ->
              None
        in
        let total_bound = LooperSummary.total_bound looper_summary.bounds in
        let big_o = EdgeExp.big_o total_bound in
        let exec_cost : Jsonbug_t.exec_cost_t = {cost= EdgeExp.to_string total_bound; big_o} in
